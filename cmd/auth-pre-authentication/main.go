// Package main is the composition root for the Cognito PreAuthentication trigger Lambda.
package main
...
func main() {
	...
	uc := auth.NewPreAuthenticationUseCase(userRepo)

	lambda.Start(func(ctx context.Context, event events.CognitoEventUserPoolsPreAuthentication) (events.CognitoEventUserPoolsPreAuthentication, error) {
		defer func() {
			if r := recover(); r != nil {
				slog.Default().ErrorContext(ctx, "panic in pre-authentication",
					slog.Any("panic", r),
