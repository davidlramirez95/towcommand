// Package main is the composition root for the provider availability toggle Lambda.
package main
...
func main() {
	...
	uc := provider.NewToggleAvailabilityUseCase(repo, repo, geo, pub, log)
	...
	h := handler.WithRecover(handler.WithCorrelationID(handler.WithLogging(
		func(ctx context.Context, event *events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
			input, err := handler.ParseBody[provider.ToggleAvailabilityInput](event)
			if err != nil {
